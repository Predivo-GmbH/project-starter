...
## Step 0.1 — DEFINE
> ⚠ **HOW this step is run is written down: see `0.1-RUN` at the BOTTOM of this file. Read it FIRST.** It was originally kept at the bottom because every step pointed into this document BY LINE NUMBER, so inserting a paragraph anywhere silently re-aimed every pointer below it — that happened on 2026-08-27 and broke eight of the nine.
>
> **That is no longer true, and the warning is corrected rather than left to frighten the next editor away from improving this file.** Since 2026-09-01 the factory's pointers are SECTION anchors, not line numbers: the live registry now stores `DESIGN#Step 0.3`, `AUDIT#Fix Loop` and the like (verified by reading `project_steps.doc_ref` on 2026-09-02 — zero bare line numbers remain). So **text may now be inserted anywhere without breaking a pointer**; what must not change is a SECTION HEADING, because that is what a pointer resolves against. `0.1-RUN` stays at the bottom because moving it would change nothing for the better.
...
3. **TOKENS (Firecrawl, ~1–2 cr).** `/v2/scrape` `formats:["branding"]` on the **CRAFT reference's** live site → `design-tokens.json` (real hex/type/radius/spacing) + deliberate identity deltas. Never eyeball values from rasters.
   > ⚠ **WHICH reference — the question that stalled this step on 2026-08-27.** A project can carry two different references and only one of them belongs here. The **CRAFT reference** is a real live page that sets the bar for type scale, spacing rhythm, colour and radius; it is picked at `reference-pick` and, per rule 3 of 0.1-RUN, **it is required on BOTH routes, animated included**. The **SUBJECT reference** may be anything at all — a video, a documented build, a knowledge-base entry — and it says what the page is ABOUT. This step scrapes the craft reference, never the subject one.
   >
   > So "the reference is a video, there is nothing to scrape" is not a gap in this step: it means a craft reference has not been picked yet. Go back to `reference-pick` rather than inventing a second source here. The knowledge base gives a METHOD and a handful of pinned values, never a complete type scale — only a measured live page yields every size, line height, tracking, spacing unit and radius at both widths. **Do NOT widen this step to allow eyeballing:** it exists because guessed values shipped before, a colour guessed as one hex turned out to be another, and that is why the scrape was made mandatory.
